...
public class ItemInHandRendererMixin {
    ...
    private static final ResourceLocation TEXTURE_LOCATION = new ResourceLocation(Thaumcraft.MODID, "textures/aspects/aer.png");
    ...
    private void renderFirstPersonItem(LivingEntity livingEntity, ItemStack itemStack, ItemTransforms.TransformType transformType, boolean leftHanded, PoseStack poseStack, MultiBufferSource buffers, int light, CallbackInfo ci) {
        if (!(livingEntity instanceof Player player) || player == null || !player.getMainHandItem().getItem().asItem().equals(ItemRegistry.THAUMOMETER.get().asItem()) || !Minecraft.getInstance().options.getCameraType().isFirstPerson()) {
            return;
        }
